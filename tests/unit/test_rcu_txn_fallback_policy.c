...
#define URCU_TXN_MW_FALLBACK_PER_COST_NUM	MAX_EXACT_SCALE
#define URCU_TXN_MW_FALLBACK_PER_COST_DEN	MAX_EXACT_SCALE
...
int main(void)
{
	...
	txn.last_cost = 2;
	ok(urcu_txn_mw__fallback_at(&txn) == URCU_TXN_MW_FALLBACK_MIN,
		"the largest exact scale is divided before applying the floor");

	txn.last_cost = UINT_MAX;
	ok(urcu_txn_mw__fallback_at(&txn) == URCU_TXN_MW_FALLBACK_MAX,
