 ...
 * test_flip_latch_lf_aba.c covers the re-plant of an already-installed record
 ...
 *
 * The fix is the installer-self-settle in urcu_flip_lf_plant(): a driver that
 ...
 * urcu_flip_lf_drive_install(V) with NO settle (helpers never settle); the
 * URCU_FLIP_LF_PREINSTALL hook, firing just before the helper plants S, runs the
 * eviction and the owner's settle.  After the helper returns, slot S must hold a
 * PLAIN value (no lingering proxy).
 *
 * Build -DURCU_FLIP_LF_NO_ABA_FIX to drop the self-settle (and the install-once
...
static struct urcu_flip_lf_mcas *g_V;
static int g_fired;
...
static struct urcu_flip_lf_mcas *make_txn(void)
{
	...
	urcu_flip_lf_mcas_add(t, AUX, &o_aux_old, &o_aux_new);
	urcu_flip_lf_mcas_add(t, S, S_OLD, S_NEW);
...
int main(void)
{
	...
	diag("final S: %s (raw=%p, S_OLD=%p, tag(S-rec) is a proxy)",
		urcu_flip_lf_is_proxy(final_s) ? "PROXY (BUG: lingering past reclaim)" :
		final_s == S_OLD ? "S_OLD (correct: FAILED, settled to old)" : "other plain",
		final_s, S_OLD);
	ok(!urcu_flip_lf_is_proxy(final_s),
