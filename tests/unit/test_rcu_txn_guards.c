...
/* Bit 0 SET: indistinguishable from a parked proxy under URCU_MCAS_TAG. */
...
static void body_sw_inline_install(void)
{
	...
	(void) urcu_txn_sw_record(&t, &g_a, NULL, V0, URCU_MCAS_TAG);
	(void) urcu_txn_sw_record(&t, &g_b, NULL, V1, URCU_MCAS_TAG);
