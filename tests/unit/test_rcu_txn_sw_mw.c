 ...
 * front-end <urcu/rcu-txn-sw-mw.h>.
 *
 ...
 *             sw-mw-aware urcu_txn_sw_mw_commit() -- the pure multi-writer path.
 *   - all-sw: each worker transfers among ITS OWN words, store_sw-only, committed
 *             through the branch-lean urcu_txn_sw_mw_commit_sw(); it must never
 *             contention-abort.
 *
...
#define URCU_TXN_SW_MW_ESCALATE	4

#include <urcu/rcu-txn-sw-mw.h>
...
static struct urcu_txn_sw_mw_domain g_domain;
...
static void *mixed_worker(void *arg)
{
	...
	rcu_register_thread();
	urcu_txn_sw_mw_init(&txn, &g_domain);
	for (n = 0; n < wa->ops; n++) {
		...
		for (;;) {
			...
			osw = (uintptr_t) urcu_txn_sw_mw_load(&txn, &g_sw[wa->tid], TAG);
			omw = (uintptr_t) urcu_txn_sw_mw_load(&txn, &g_mw[j], TAG);
			urcu_txn_sw_mw_store_sw(&txn, &g_sw[wa->tid],
				(void *) osw, (void *) lf_bump(osw, d), TAG);
			urcu_txn_sw_mw_store_mw(&txn, &g_mw[j],
...
static void *allmw_worker(void *arg)
{
	...
	for (n = 0; n < wa->ops; n++) {
		...
		for (;;) {
			...
			urcu_txn_sw_mw_begin(&txn);
			oi = (uintptr_t) urcu_txn_sw_mw_load(&txn, &g_mw[i], TAG);
			oj = (uintptr_t) urcu_txn_sw_mw_load(&txn, &g_mw[j], TAG);
			urcu_txn_sw_mw_store_mw(&txn, &g_mw[i],
...
static void *allsw_worker(void *arg)
{
	...
	for (n = 0; n < wa->ops; n++) {
		...
		urcu_txn_sw_mw_begin(&txn);
		oa = (uintptr_t) urcu_txn_sw_mw_load(&txn, &g_priv[wa->tid][a], TAG);
		ob = (uintptr_t) urcu_txn_sw_mw_load(&txn, &g_priv[wa->tid][b], TAG);
		urcu_txn_sw_mw_store_sw(&txn, &g_priv[wa->tid][a],
			(void *) oa, (void *) lf_bump(oa, d), TAG);
		urcu_txn_sw_mw_store_sw(&txn, &g_priv[wa->tid][b],
...
static void functional_checks(void)
{
	...
	urcu_txn_sw_mw_init(&txn, NULL);
	urcu_txn_sw_mw_begin(&txn);
	urcu_txn_sw_mw_store_sw(&txn, &sa, (void *) 0x100, (void *) 0x110, TAG);
	urcu_txn_sw_mw_store_sw(&txn, &sb, (void *) 0x200, (void *) 0x210, TAG);
	urcu_txn_sw_mw_store_mw(&txn, &ma, (void *) 0x300, (void *) 0x310, TAG);
	urcu_txn_sw_mw_store_mw(&txn, &mb, (void *) 0x400, (void *) 0x410, TAG);
	st = urcu_txn_sw_mw_commit(&txn);
	urcu_txn_sw_mw_end(&txn);
	...
	urcu_txn_sw_mw_begin(&txn);
	urcu_txn_sw_mw_store_sw(&txn, &sa, (void *) 0x10, (void *) 0x1a, TAG);
	urcu_txn_sw_mw_store_sw(&txn, &sb, (void *) 0x20, (void *) 0x2a, TAG);
	st = urcu_txn_sw_mw_commit_sw(&txn);
	urcu_txn_sw_mw_end(&txn);
	...
	urcu_txn_sw_mw_begin(&txn);
	urcu_txn_sw_mw_store_sw(&txn, &sa, (void *) 0xA0, (void *) 0xA2, TAG);
	urcu_txn_sw_mw_store_mw(&txn, &ma, (void *) 0xBAD0, (void *) 0xB2, TAG);
	st = urcu_txn_sw_mw_commit(&txn);
	urcu_txn_sw_mw_end(&txn);
	...
	urcu_txn_sw_mw_begin(&txn);
	urcu_txn_sw_mw_store_sw(&txn, &sa, (void *) 0xA0, (void *) 0xA2, TAG);
	urcu_txn_sw_mw_store_mw(&txn, &ma, (void *) 0xB0, (void *) 0xB2, TAG);
	st = urcu_txn_sw_mw_commit(&txn);
	urcu_txn_sw_mw_end(&txn);
	...
	urcu_txn_sw_mw_begin(&txn);
	urcu_txn_sw_mw_store_sw(&txn, &sa, (void *) 0x1000, (void *) 0x1002, TAG);
	urcu_txn_sw_mw_store_mw(&txn, &ma, (void *) 0x2000, (void *) 0x2002, TAG);
	{
		void *rsw = urcu_txn_sw_mw_load(&txn, &sa, TAG);
		void *rmw = urcu_txn_sw_mw_load(&txn, &ma, TAG);

		ok(rsw == (void *) 0x1002 && rmw == (void *) 0x2002,
			"read-your-own-writes returns the pending value for SW and MW");
	}
	(void) urcu_txn_sw_mw_commit(&txn);
	urcu_txn_sw_mw_end(&txn);
}
...
int main(void)
{
	...
	plan_tests(NR_TESTS);
	rcu_register_thread();
	urcu_txn_sw_mw_domain_init(&g_domain);
