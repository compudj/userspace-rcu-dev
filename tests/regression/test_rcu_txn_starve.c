 ...
 *     commits with a bare CAS until it has retried past URCU_TXN_ESCALATE --
 ...
 */
static inline void sample_lane(struct urcu_txn *txn, struct stats *st,
		int *in, int *pub)
{
 ...
 */
static inline int attempt_done(struct urcu_txn *txn, struct stats *st,
...
static void *small_writer(void *p)
{
	...
	while (!uatomic_load(&g_stop, CMM_RELAXED)) {
		...
		do {
			...
			v = urcu_txn_load(&txn, &slot[i], URCU_TXN_TAG);
			urcu_txn_store_mw(&txn, &slot[i], v,
					(void *) ((uintptr_t) v + 2),
					URCU_TXN_TAG);
...
static void *wide_writer(void *p)
{
	...
	while (!uatomic_load(&g_stop, CMM_RELAXED)) {
		...
		do {
			...
			for (k = 0; k < width; k++) {
				void **s = &slot[pick[k]];
				void *v = urcu_txn_load(&txn, s, URCU_TXN_TAG);

				urcu_txn_store_mw(&txn, s, v,
						(void *) ((uintptr_t) v + 2),
						URCU_TXN_TAG);
			}
			st = urcu_txn_commit(&txn);
...
int main(int argc, char *argv[])
{
	...
	diag("wide op: cost %d (loads %d + records %d) => budget %lu retries  |  URCU_TXN_ESCALATE=%d",
		2 * width, width, width, wide_budget(), URCU_TXN_ESCALATE);
