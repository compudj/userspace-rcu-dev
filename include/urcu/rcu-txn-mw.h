...
#ifndef _URCU_RCU_TXN_MW_H
#define _URCU_RCU_TXN_MW_H
 ...
 * retry bookkeeping.  Loads inside the bracket go through urcu_txn_mw_load(): it
 ...
 * are buffered:
 *
 *     struct urcu_txn_mw_domain domain;   // once, shared per structure
 *     urcu_txn_mw_domain_init(&domain);
 *     ...
 *     struct urcu_txn_mw txn;
 *     enum urcu_txn_status st;
 *
 *     urcu_txn_mw_init(&txn, &domain);    // or NULL: no fallback
 *     do {
 ...
 *         succ = urcu_txn_mw_load(&txn, (void **) &pos->next, TAG);
 *         if (is_marked(succ)) { urcu_txn_mw_end(&txn); return -ENOENT; }
 *         urcu_txn_mw_store(&txn, (void **) &pos->next, succ, newp, TAG);
 *         urcu_txn_mw_store(&txn, (void **) &succ->prev, pos,  newp, TAG);
 ...
 *
 * Writes.  urcu_txn_mw_store() buffers a write whose old the caller supplies.
 ...
 *
 * Read-your-own-writes is the DEFAULT: urcu_txn_mw_load() returns a slot's pending
 ...
 *
 * Two opt-outs.  urcu_txn_mw_declare_disjoint() asserts the write set touches
 ...
 * urcu_txn_mw_load_committed() is the per-read escape hatch, returning a slot's
 * committed value even for one this attempt has written.
 *
 * Reserve.  A mutator that knows its edge count up front may call
 * urcu_txn_mw_reserve() right after begin: it allocates the descriptor to that
 ...
 *
 * Guards.  urcu_txn_mw_load_validate() folds a READ into the commit's conflict set
 ...
 * (see URCU_TXN_MW_FALLBACK_PER_COST_NUM).  Escalation is purely reactive: a
 ...
 * initiator (urcu_txn_mw__maybe_publish), so the funnel persists exactly as long
 ...
 * an ABORT calls urcu_txn_mw_abandon() before end().  A departing initiator clears
 ...
 * transactions.
 */
#ifndef URCU_TXN_MW_INIT
#define URCU_TXN_MW_INIT	4
#endif
 ...
 *
 *     budget = PER_COST * cost,   capped at URCU_TXN_MW_FALLBACK_MAX
 *
 ...
 * cost.  Set PER_COST_NUM to 0 to select a flat URCU_TXN_MW_FALLBACK budget
 * instead (the cost accounting then compiles out entirely).
 */
#ifndef URCU_TXN_MW_FALLBACK_PER_COST_NUM
#define URCU_TXN_MW_FALLBACK_PER_COST_NUM	11	/* 11/4 = 2.75 */
#endif
#ifndef URCU_TXN_MW_FALLBACK_PER_COST_DEN
#define URCU_TXN_MW_FALLBACK_PER_COST_DEN	4
#endif
#ifndef URCU_TXN_MW_FALLBACK		/* only consulted when PER_COST_NUM == 0 */
#define URCU_TXN_MW_FALLBACK		256
#endif
/*
 ...
 * and no higher.
 */
#ifndef URCU_TXN_MW_FALLBACK_MIN
#define URCU_TXN_MW_FALLBACK_MIN		64
#endif
#ifndef URCU_TXN_MW_FALLBACK_MAX
#define URCU_TXN_MW_FALLBACK_MAX		4096
#endif

urcu_static_assert(URCU_TXN_MW_FALLBACK_PER_COST_NUM >= 0,
		"URCU_TXN_MW_FALLBACK_PER_COST_NUM must not be negative",
		urcu_txn_mw_fallback_per_cost_num_nonnegative);
urcu_static_assert(URCU_TXN_MW_FALLBACK_PER_COST_NUM <= UINT64_MAX / UINT_MAX,
		"URCU_TXN_MW_FALLBACK_PER_COST_NUM is too large for exact scaling",
		urcu_txn_mw_fallback_per_cost_num_fits_product);
urcu_static_assert(URCU_TXN_MW_FALLBACK_PER_COST_DEN > 0,
		"URCU_TXN_MW_FALLBACK_PER_COST_DEN must be greater than zero",
		urcu_txn_mw_fallback_per_cost_den_positive);
urcu_static_assert(URCU_TXN_MW_FALLBACK_MIN <= URCU_TXN_MW_FALLBACK_MAX,
		"URCU_TXN_MW_FALLBACK_MIN must not exceed URCU_TXN_MW_FALLBACK_MAX",
		urcu_txn_mw_fallback_bounds_ordered);

/*
 * Read-your-own-writes lookup filter (a Bloom word), maintained for every
 * transaction except one that declared its write set disjoint.  urcu_txn_mw__load
 ...
 * per-transaction at runtime via urcu_txn_mw_declare_disjoint() /
 * urcu_txn_mw_expect_conflict().
 */
 ...
 * urcu_txn_mw_expect_conflict() so their first attempt goes straight to the find
 ...
 * urcu_txn_mw_read_lock()/urcu_txn_mw_read_unlock() (hence begin()/end()) ONLY when a
 * handle binds no flavor (urcu_txn_mw_init's NULL).  Defaults to the
 ...
 * its RCU flavor at runtime) is to bind that flavor with urcu_txn_mw_init_flavor()
 * so the bracket opens in it directly; this macro then never fires.
 */
#ifndef URCU_TXN_MW_RCU_READ_LOCK
#define URCU_TXN_MW_RCU_READ_LOCK()	rcu_read_lock()
#endif
#ifndef URCU_TXN_MW_RCU_READ_UNLOCK
#define URCU_TXN_MW_RCU_READ_UNLOCK()	rcu_read_unlock()
#endif
 ...
 */
#define URCU_TXN_MW_ENOMEM	((struct urcu_mcas *) -1L)
 ...
 *
 * urcu_txn_mw_load_optimistic() resolves an undecided parker to its logical old --
 ...
 * load.  Build
 * with -DURCU_TXN_DEBUG_READ_POLICY to enforce it -- every urcu_txn_mw__record()
 ...
 * on-stack handle by URCU_TXN_MW_RP_SLOTS entries.  Debug builds only.
 *
 ...
 * no read to speak of.  Note that urcu_txn_mw_load_validate_optimistic() is a
 * read-set read that does not wait, so it violates the policy BY CONSTRUCTION
 * -- it has no callers, and this is why.
 */
#ifdef URCU_TXN_MW_DEBUG_READ_POLICY
# include <stdio.h>
# include <stdlib.h>
# include <string.h>
# ifndef URCU_TXN_MW_RP_SLOTS
#  define URCU_TXN_MW_RP_SLOTS	256	/* Power of two. */
# endif
# define URCU_TXN_MW_RP_PROBE	8	/* Linear probe before evicting. */
struct urcu_txn_mw__rp_entry {
...
struct urcu_txn_mw {
	struct urcu_txn_mw_domain *domain;	/* Escalation domain, or NULL */
	const struct rcu_flavor_struct *flavor;	/*
						 * RCU flavor for the read-side
						 * bracket, or NULL to use the
						 * compile-time-selected flavor
						 * (URCU_TXN_MW_RCU_READ_LOCK).
						 * Set at create by
						 * urcu_txn_mw_init_flavor(); lets
	...
	int disjoint;			/*
					 ...
					 * urcu_txn_mw_declare_disjoint() before
					 * begin(); a violation corrupts (adds a
					 * duplicate record) unless
					 * URCU_TXN_MW_DEBUG_DISJOINT traps it.
					 */
	unsigned int nload;		/*
	...
	int expect_conflict;		/*
					 ...
					 * urcu_txn_mw_expect_conflict() before
					 * begin().
					 */
	int esc_pending;		/*
					 ...
					 */
#ifdef URCU_TXN_MW_ESCALATION_STATS
	/*
...
#endif
#ifdef URCU_TXN_MW_DEBUG_READ_POLICY
	struct urcu_txn_mw__rp_entry rp[URCU_TXN_MW_RP_SLOTS];
					/*
...
static inline
unsigned int urcu_txn_mw__rp_hash(void **slot)
{
	uintptr_t h = (uintptr_t) slot >> 3;	/* slots are pointer-aligned */

	h *= 0x9e3779b97f4a7c15ULL;
	return (unsigned int) ((h >> 40) & (URCU_TXN_MW_RP_SLOTS - 1));
}

/* A new attempt reloads everything: forget every load. */
static inline
void urcu_txn_mw__rp_reset(struct urcu_txn_mw *txn)
{
...
static inline
void urcu_txn_mw__rp_note(struct urcu_txn_mw *txn, void **slot, int optimistic)
{
	unsigned int h = urcu_txn_mw__rp_hash(slot), i;

	for (i = 0; i < URCU_TXN_MW_RP_PROBE; i++) {
		struct urcu_txn_mw__rp_entry *e =
			&txn->rp[(h + i) & (URCU_TXN_MW_RP_SLOTS - 1)];
 ...
 * load (urcu_txn_mw_load / _load_validate), not an optimistic one.  A
 ...
 */
static inline
void urcu_txn_mw__rp_check(struct urcu_txn_mw *txn, void **slot)
{
	unsigned int h = urcu_txn_mw__rp_hash(slot), i;
	...
	for (i = 0; i < URCU_TXN_MW_RP_PROBE; i++) {
		...
		e->optimistic = 0;
		txn->rp_violations++;
#ifndef URCU_TXN_MW_DEBUG_READ_POLICY_SOFT
		fprintf(stderr, "urcu-txn: read-policy violation: slot %p enters "
			...
			"parker commits.  Use urcu_txn_mw_load()/urcu_txn_mw_load_validate() "
			"for a slot this transaction stores or guards; keep "
			"urcu_txn_mw_load_optimistic() for navigation.\n", (void *) slot);
		abort();
#endif
		return;
	}
 ...
 */
static inline
unsigned long urcu_txn_mw_read_policy_violations(const struct urcu_txn_mw *txn)
{
	return txn->rp_violations;
}

static inline
unsigned long urcu_txn_mw_read_policy_evicted(const struct urcu_txn_mw *txn)
{
	return txn->rp_evicted;
}

#else	/* !URCU_TXN_MW_DEBUG_READ_POLICY */

# define urcu_txn_mw__rp_reset(txn)		do { } while (0)
# define urcu_txn_mw__rp_note(txn, slot, opt)	do { } while (0)
# define urcu_txn_mw__rp_check(txn, slot)		do { } while (0)
...
#endif	/* URCU_TXN_MW_DEBUG_READ_POLICY */

/*
 * Empty this attempt's read-your-own-writes filter.  Called at the ONE point an
 * attempt's descriptor goes from "none" to "live" (urcu_txn_mw_reserve,
 * urcu_txn_mw__record) -- which is strictly before the filter can be consulted,
 * since urcu_txn_mw__load and urcu_txn_mw__record both test it only once txn->mcas is
 ...
 * commit_flavor; see urcu_txn_mw_read_lock) sound: that mode has no begin() to
 ...
 */
static inline
void urcu_txn_mw__bloom_reset(struct urcu_txn_mw *txn)
{
	if (!txn->disjoint)
 ...
 * flavor (the URCU_TXN_MW_RCU_READ_LOCK default) -- the plain urcu_txn_mw_init() does
 ...
 * that flavor here so begin()/end() -- and the standalone urcu_txn_mw_read_lock()
 * / urcu_txn_mw_read_unlock() bracket -- open the read-side section in it, the
 * same way commit_flavor() defers reclaim through flavor->update_call_rcu.
 */
static inline
void urcu_txn_mw_init_flavor(struct urcu_txn_mw *txn,
		...
		const struct rcu_flavor_struct *flavor)
{
	 ...
	 * urcu_txn_mw__bloom_reset() at the point the attempt's descriptor is
	...
	txn->rp_violations = 0;
	txn->rp_evicted = 0;
	urcu_txn_mw__rp_reset(txn);
#endif
}

#ifdef URCU_TXN_MW_ESCALATION_STATS
static inline
unsigned int urcu_txn_mw_esc_raw(const struct urcu_txn_mw *txn) { return txn->esc_raw; }
static inline
unsigned int urcu_txn_mw_esc_waw(const struct urcu_txn_mw *txn) { return txn->esc_waw; }
static inline
unsigned int urcu_txn_mw_esc_bloom(const struct urcu_txn_mw *txn) { return txn->esc_bloom; }
#endif
 ...
 * by construction rather than by luck, both want urcu_txn_mw_expect_conflict() so
 ...
 */
static inline
void urcu_txn_mw_declare_disjoint(struct urcu_txn_mw *txn)
{
	txn->disjoint = 1;
}
 ...
 */
static inline
void urcu_txn_mw_expect_conflict(struct urcu_txn_mw *txn)
{
 ...
 */
static inline
unsigned long urcu_txn_mw__eff_retry(const struct urcu_txn_mw *txn)
{
...
static inline
void urcu_txn_mw_init(struct urcu_txn_mw *txn,
		struct urcu_txn_mw_domain *domain)
{
	urcu_txn_mw_init_flavor(txn, domain, NULL);
}

/*
 * Open / close the txn's RCU read-side section through its bound flavor (set by
 * urcu_txn_mw_init_flavor), falling back to the compile-time-selected flavor when
 ...
 */
static inline
void urcu_txn_mw_read_lock(struct urcu_txn_mw *txn)
{
	if (txn->flavor)
...
static inline
void urcu_txn_mw_read_unlock(struct urcu_txn_mw *txn)
{
	if (txn->flavor)
		txn->flavor->read_unlock();
	else
		URCU_TXN_MW_RCU_READ_UNLOCK();
}
 ...
 * guard-driven urcu_txn_mw_conflict() path, which never reaches commit at all.
 ...
 */
static inline
unsigned int urcu_txn_mw__attempt_cost(const struct urcu_txn_mw *txn)
{
	const struct urcu_mcas *m = txn->mcas;
	unsigned int w = (m && m != URCU_TXN_MW_ENOMEM) ? m->nr : 0;
 ...
 */
static inline
void urcu_txn_mw__learn_cost(struct urcu_txn_mw *txn)
{
#if URCU_TXN_MW_FALLBACK_PER_COST_NUM
	unsigned int c = urcu_txn_mw__attempt_cost(txn);
 ...
 * lane. With URCU_TXN_MW_FALLBACK_PER_COST_NUM == 0 this is the flat
 * URCU_TXN_MW_FALLBACK. Otherwise it scales with the operation's high-water cost
 ...
 * URCU_TXN_MW_FALLBACK_PER_COST_NUM comment above.
 */
static inline
unsigned long urcu_txn_mw__fallback_at(const struct urcu_txn_mw *txn)
{
	const uint64_t num = URCU_TXN_MW_FALLBACK_PER_COST_NUM;
	/* Keep the arithmetic well-formed for the flat and rejected configurations. */
	const uint64_t den = URCU_TXN_MW_FALLBACK_PER_COST_DEN > 0 ?
		URCU_TXN_MW_FALLBACK_PER_COST_DEN : 1;
	uint64_t n, t;
	...
	t = (num * n) / den;
	if (t < URCU_TXN_MW_FALLBACK_MIN)
		t = URCU_TXN_MW_FALLBACK_MIN;	/* also rules out a zero budget */
	return t > URCU_TXN_MW_FALLBACK_MAX ? URCU_TXN_MW_FALLBACK_MAX : t;
}

static inline
int urcu_txn_mw__self_qualifies(const struct urcu_txn_mw *txn)
{
	return txn->retry >= urcu_txn_mw__fallback_at(txn);
}

static inline
void urcu_txn_mw__enter_fallback(struct urcu_txn_mw *txn)
{
	/*
	 ...
	 */
	if (urcu_txn_mw__self_qualifies(txn)) {
 ...
 */
static inline
void urcu_txn_mw__exit_fallback(struct urcu_txn_mw *txn)
{
	/*
	 ...
	 * suppresses escalation on it and urcu_txn_mw_end() unlocks on it.
 ...
 */
static inline
void urcu_txn_mw__maybe_publish(struct urcu_txn_mw *txn)
{
	if (txn->domain && uatomic_load(&txn->in_fallback, CMM_RELAXED) &&
			!txn->fb_published && urcu_txn_mw__self_qualifies(txn)) {
		uatomic_store(&txn->domain->active, 1, CMM_RELAXED);
		txn->fb_published = 1;
	}
}
...
static inline
void urcu_txn_mw_begin(struct urcu_txn_mw *txn)
{
	txn->retrying = 0;
	 ...
	 */
	if (urcu_txn_mw__want_fallback(txn))
		urcu_txn_mw__enter_fallback(txn);
	else
		urcu_txn_mw__maybe_publish(txn);	/* joiner that starved: promote */
	 ...
	 * urcu_txn_mw__bloom_reset() when the next store (or reserve) allocates
	...
	txn->esc_raw = txn->esc_waw = txn->esc_bloom = 0;
#endif
	urcu_txn_mw__rp_reset(txn);	/* debug: a new attempt reloads every slot */
 ...
 */
static inline
int urcu_txn_mw_reserve(struct urcu_txn_mw *txn, unsigned int n)
{
	struct urcu_mcas *m;

	txn->min_alloc = n;
	if (caa_unlikely(txn->mcas == URCU_TXN_MW_ENOMEM))
	...
	if (!txn->mcas) {
		 ...
		 */
		m = urcu_mcas_create(n, urcu_txn_mw__eff_retry(txn));
		if (caa_unlikely(!m)) {
			txn->mcas = URCU_TXN_MW_ENOMEM;
			return -ENOMEM;
		}
		urcu_txn_mw__bloom_reset(txn);	/* the attempt's write set opens empty */
		txn->mcas = m;
		return 0;
	}
 ...
 */
static inline
bool urcu_txn_mw__reconcile(struct urcu_txn_mw *txn, struct urcu_mcas *m,
 ...
 */
static inline
int urcu_txn_mw__record(struct urcu_txn_mw *txn, void **slot,
		void *old_ptr, void *new_ptr, int upgrade, uintptr_t tag)
{
	struct urcu_mcas *m = txn->mcas;

	urcu_txn_mw__rp_check(txn, slot);	/* debug: @slot enters the read/write set */
	if (caa_unlikely(m == URCU_TXN_MW_ENOMEM))
		return -ENOMEM;		/* sticky: an earlier record already failed */
	if (!m) {
		m = urcu_mcas_create(txn->min_alloc ? txn->min_alloc :
				URCU_TXN_MW_INIT, urcu_txn_mw__eff_retry(txn));
		if (caa_unlikely(!m)) {
			txn->mcas = URCU_TXN_MW_ENOMEM;
			return -ENOMEM;
		}
	...
	if (!txn->disjoint) {
		...
		if (coincide && urcu_txn_mw__eff_retry(txn) == 0)
			txn->esc_pending = 1;
#ifdef URCU_TXN_MW_ESCALATION_STATS
		if (coincide)
...
#endif
	{
		 ...
		 *    URCU_TXN_MW_DEBUG_DISJOINT traps it here.
		 */
		if (urcu_txn_mw__eff_retry(txn) == 0) {
#ifdef URCU_TXN_MW_DEBUG_DISJOINT
			if (txn->disjoint && urcu_mcas_find(m, slot) != NULL) {
				fprintf(stderr, "urcu-txn: disjoint-contract violation: "
					...
					"urcu_txn_mw_declare_disjoint().  The blind append would "
			...
			recorded = urcu_txn_mw__reconcile(txn, m, slot, old_ptr,
					new_ptr, upgrade, tag);
		if (caa_unlikely(!recorded)) {
			...
			if (urcu_txn_mw__eff_retry(txn) == 0)
				...
				urcu_txn_mw__reconcile(txn, m, slot, old_ptr,
						new_ptr, upgrade, tag);
		}
	}
	return 0;
 ...
 */
static inline
void *urcu_txn_mw__load(struct urcu_txn_mw *txn, void **slot, uintptr_t tag,
		int optimistic, int committed)
{
...
#endif
#ifndef URCU_TXN_MW_RYW_NO_BLOOM
		/*
	...
	urcu_txn_mw__rp_note(txn, slot, optimistic);	/* debug: read-policy */
	return optimistic ? urcu_mcas_read_optimistic(slot, tag)
			: urcu_mcas_read(slot, tag);
}

static inline
void *urcu_txn_mw_load(struct urcu_txn_mw *txn, void **slot, uintptr_t tag)
{
	return urcu_txn_mw__load(txn, slot, tag, 0, 0);
}

/*
 * urcu_txn_mw_load that ignores this attempt's own buffered stores and returns
 ...
 */
static inline
void *urcu_txn_mw_load_committed(struct urcu_txn_mw *txn, void **slot,
		uintptr_t tag)
{
	return urcu_txn_mw__load(txn, slot, tag, 0, 1);
}

/*
 * urcu_txn_mw_load without waiting on an undecided transaction to decide: forwards
 ...
 * urcu_txn_mw_load -- a value that moved aborts.  A stale read costs an abort, not
 ...
 * at commit anyway.  Keep urcu_txn_mw_load() for a value the op must see settled
 * at the point it reads it.  RYW is honoured identically.
 */
static inline
void *urcu_txn_mw_load_optimistic(struct urcu_txn_mw *txn, void **slot,
		uintptr_t tag)
{
	return urcu_txn_mw__load(txn, slot, tag, 1, 0);
}

/*
 * urcu_txn_mw_load_committed that reads optimistically (see
 * urcu_txn_mw_load_optimistic).  Same committed-view, read-only contract.
 */
static inline
void *urcu_txn_mw_load_committed_optimistic(struct urcu_txn_mw *txn, void **slot,
		uintptr_t tag)
{
	return urcu_txn_mw__load(txn, slot, tag, 1, 1);
}

/*
 * Read @slot like urcu_txn_mw_load AND pin it: besides returning its current
 ...
 */
static inline
void *urcu_txn_mw_load_validate(struct urcu_txn_mw *txn, void **slot,
		uintptr_t tag)
{
	void *v = urcu_txn_mw_load(txn, slot, tag);

	(void) urcu_txn_mw__record(txn, slot, v, v, 0, tag);
	return v;
}

/*
 * urcu_txn_mw_load_validate that reads optimistically (urcu_txn_mw_load_optimistic).
 ...
 */
static inline
void *urcu_txn_mw_load_validate_optimistic(struct urcu_txn_mw *txn, void **slot,
		uintptr_t tag)
{
	void *v = urcu_txn_mw_load_optimistic(txn, slot, tag);
 ...
 *
 * The difference from urcu_txn_mw_load_validate() is where the expected value
 ...
 */
static inline
void urcu_txn_mw_validate(struct urcu_txn_mw *txn, void **slot,
		void *expected, uintptr_t tag)
{
	(void) urcu_txn_mw__record(txn, slot, expected, expected, 0, tag);
}
 ...
 */
static inline
int urcu_txn_mw_store(struct urcu_txn_mw *txn, void **slot,
		void *old_ptr, void *new_ptr, uintptr_t tag)
{
	return urcu_txn_mw__record(txn, slot, old_ptr, new_ptr, 1, tag);
}
 ...
 * urcu_txn_mw_commit() passes the compile-time-selected call_rcu.  Call between
 * begin and end.
 */
static inline
enum urcu_txn_status urcu_txn_mw_commit_flavor(struct urcu_txn_mw *txn,
 ...
 * thin wrapper over urcu_txn_mw_commit_flavor(); see it for the full contract.
 */
static inline
enum urcu_txn_status urcu_txn_mw_commit(struct urcu_txn_mw *txn)
{
	return urcu_txn_mw_commit_flavor(txn, call_rcu);
}
 ...
 * urcu_txn_mw_abandon() before end() -- see it.
 */
static inline
void urcu_txn_mw_conflict(struct urcu_txn_mw *txn)
{
	urcu_txn_mw__learn_cost(txn);	/* the descent counts */
 ...
 * an ABORT (from commit, or from urcu_txn_mw_conflict()) and will NOT run another
...
static inline
unsigned int urcu_txn_mw_last_cost(const struct urcu_txn_mw *txn)
{
	return txn->last_cost;
}

static inline
void urcu_txn_mw_abandon(struct urcu_txn_mw *txn)
{
...
static inline
void urcu_txn_mw_end(struct urcu_txn_mw *txn)
{
	struct urcu_mcas *m = txn->mcas;
	...
	if (uatomic_load(&txn->in_fallback, CMM_RELAXED) && !txn->retrying)
		urcu_txn_mw__exit_fallback(txn);
}
