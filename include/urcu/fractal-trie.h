 ...
 *
 * On 64-bit ABIs the attribute is a no-op size-wise: the two void *
 * members already total 16 bytes and natural struct alignment is 8.
 * On 32-bit ABIs the attribute doubles the struct from 8 to 16 bytes;
 * the lookup hot path benefits in exchange (clean 4-bit kind nibble
 * lets the per-byte-step dispatch elide the bit-0 EXT-alternate
 * gate).
 */
struct cds_ft_node {
	/*
...
/*
 * cds_ft_alloc_external - Allocate an external-node buffer from the
 * Fractal Trie's internal arena.
 * @ft:   The Fractal Trie.
 * @size: Minimum number of bytes needed by the caller.  Rounded up
 *        internally to the next power of two; the returned buffer
 *        is at least @size bytes.  Must be non-zero and the rounded
 *        size must not exceed the system page size.
 *
 * The returned buffer is suitable for use as an external node passed
 * to the cds_ft_insert_* family of functions.  The application places
 * its struct cds_ft_node anywhere within the buffer; the
 * ext_node_offset attribute set at trie-creation time tells the trie
 * where to find it.
 *
 * Co-locating external nodes with internal trie nodes in the same
 ...
 *
 * Allocations are protected by an arena-level mutex shared with
 * internal-node allocations of the same rounded size.  Callers that
 * generate a high concurrent insert rate may see additional
 * contention.
 */
void *cds_ft_alloc_external(struct cds_ft *ft, size_t size);

/*
 * cds_ft_free_external - Release an external-node buffer back to the
 * Fractal Trie's internal arena, deferred via call_rcu.
 * @p: Buffer previously returned by cds_ft_alloc_external (or NULL).
 *
 * The buffer must have been removed from the trie before calling
 * this function (the caller is responsible for any prior
 * cds_ft_remove / cds_ft_detach).  Concurrent RCU readers may still
 * dereference @p; the actual reclamation is deferred to the next
 * grace period using the RCU flavor that was registered on the
 * trie's group.
 *
 * The arena and RCU flavor are derived from @p, so no trie pointer
 * is required.
 */
void cds_ft_free_external(void *p);

/*
 * cds_ft_free_external_unpublished - Release an external-node buffer
 * synchronously, without RCU deferral.
 * @p: Buffer previously returned by cds_ft_alloc_external (or NULL).
 *
 * The caller asserts that no concurrent reader can hold a reference
 * to @p.  This holds when @p was never published into a trie (e.g.
 * a duplicate dropped during merge), when the originating trie is
 * in exclusive mode, or when the caller has already synchronized
 * an RCU grace period after detaching the node.
 *
 * Misuse — calling this while RCU readers can still observe @p —
 * is a use-after-free bug.  When in doubt, use cds_ft_free_external.
 */
void cds_ft_free_external_unpublished(void *p);
 ...
 *
 * Lookups via cds_ft_lookup_key (and the iterator-based
 * cds_ft_lookup) on this group's tries descend speculatively and
 * validate the result against the external node's stored key using
 * the library's inline SIMD/SWAR comparator before returning.  This
 * gives the same verified-result contract as a precise lookup, with
 ...
 * -DFEATURE_FT_VERIFY_AT_MUTATION, in which case the writer scope-exit
 * hook samples cds_ft_verify at a per-trie tunable period (see
 * cds_ft_verify_at_mutation_period_set).  Returns false if the feature
 * is compiled out (the default), in which case the period setter /
 * getter return CDS_FT_STATUS_NOT_SUPPORTED.
 *
