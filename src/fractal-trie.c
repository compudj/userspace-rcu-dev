 ...
 *                  resolve their own slot via skip_slot_offset.
 */
static
		...
		struct cds_ft_inode_flag **slot_ptr)
{
	...
	surviving_child = ft_node_get_minmax(iter_node_flag,
	...
	if (parent_cn) {
		...
		publish_slot = ft_get_skip_slot(parent_cn_meta, ft);
	} else {
		new_cn_meta->parent = iter_meta->parent;
		publish_parent = iter_meta->parent;
		publish_slot = slot_ptr;
	}
	ft_set_skip_slot(new_cn_meta, publish_slot);

	new_cn_flag = ft_compressed_node_flag(new_cn);
	ft_set_parent(new_cn->child, new_cn_flag, &new_cn->child);
		...
		*result_node = external_nodes;
		...
		if (metadata->nr_child == 0 && dd.d.pnfp != NULL) {
			...
			if (ret) {
			 ...
			 metadata->parent != NULL) {
			/*
			 * Surviving 1-child internal at non-root, no
			 * external_nodes: collapse via chain-compress to
			 * restore the SKIP_COMPRESSED invariant.
			 */
			ft_canonicalize_chain_compress(ft, dd.d.nf,
				metadata, dd.d.nfp);
		}
 ...
 */
static
bool ft_graft_glue_is_fresh(struct ft_graft_glue *g,
		struct cds_ft_inode_flag *child)
{
	void *cp;
	int i;
	...
	if (ft_node_compressed(child))
		...
		cp = ft_skip_to_compressed(child);
		...
		bool dst_origin)
{
	...
	if (ft_graft_glue_is_fresh(g, child)) {
		ft_set_parent(child, parent, slot);
		return;
	}
		...
		struct ft_graft_glue *glue)
{
	...
	if (d->depth == key_len) {
		 ...
		 *       the skip_slot_offset is fixed up by ft_node_set_nth's
	...
	case FT_GRAFT_SWAP_EXACT:
		*off_ret = 0;
		if (ft_node_external(d->nf))
			...
			*count_ret = ft_nr_keys_get(ft_flag_to_metadata(d->nf));
		break;
		...
		struct cds_ft_inode_flag *M, unsigned long mk)
{
	...
	if (ft_node_compressed(M)) {
		 ...
		 */
		ft_graft_glue_defer_edge_origin(c->gd, mcn->child, mflag,
				&merged->child, /*dst_origin=*/ true);
		ft_graft_glue_untrack(c->gd, mcn);
