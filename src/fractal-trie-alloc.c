...
/*
 * Round up size to the next power-of-two and return its order
 * (i.e. the minimum n such that size <= (1 << n)).
 */
static
size_t ft_size_to_order(size_t size)
{
	if (size <= 1)
		return 0;
	return (sizeof(unsigned long) * 8) - __builtin_clzl(size - 1);
}
...
void *cds_ft_alloc_external(struct cds_ft *ft, size_t size)
{
	...
	if (!cds_ft_page_size)
		cds_ft_page_size = urcu_get_page_len();
	order = ft_size_to_order(size);
	if ((1UL << order) > cds_ft_page_size) {
		errno = EINVAL;
		return NULL;
	}
...
void cds_ft_free_external(void *p)
{
	...
	if (!p)
		return;
	metadata = cds_ft_item_to_metadata(p);
#ifdef FT_IMMEDIATE_FREE
	cds_ft_do_free_item(metadata);
#else
	{
		struct cds_ft_metadata_alloc *metadata_alloc =
			caa_container_of(metadata,
					struct cds_ft_metadata_alloc, metadata);
		struct cds_ft_alloc_arena *arena =
			cds_ft_metadata_to_range(metadata)->arena;

		arena->ft_group->flavor->update_call_rcu(
				&metadata_alloc->rcu_head,
				cds_ft_free_item_rcu);
	}
#endif
}
...
void cds_ft_free_external_unpublished(void *p)
{
	if (!p)
		return;
	cds_ft_do_free_item(cds_ft_item_to_metadata(p));
}
