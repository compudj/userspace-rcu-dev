...
struct cds_ft_qp16_node {
	...
	struct cds_ft_inode_flag *ptrs[];	/* bytes 8+: popcount(bitmap) entries
						 * (direct variant). Skip variant
						 * shifts ptrs[] to byte 16; see
						 * ft_qp16_ptrs(node, is_skip). */
} __attribute__((__aligned__(8)));

#ifdef FEATURE_FT_SKIP_COMPRESSED
/*
 * Skip-variant header view, anchored at byte 2 of a QP node:
 *
 *   skip_len    : same byte as cds_ft_qp16_node::skip_len
 *   subkey[13]  : skip-variant cached subkey, bytes 3-15.  First 5 bytes
 *                 alias cds_ft_qp16_node::subkey; bytes 5-12 occupy the
 *                 overlay (where direct variant carries ptrs[0]).
 *
 * Only valid to access this view when the slot tag that pointed at the
 * node is FT_KIND_SKIP_QP.  Reading via this view on a direct-variant
 * node returns subkey[5..12] = the first ptr (interpreted as bytes),
 * which is nonsense — never do it.
 */
